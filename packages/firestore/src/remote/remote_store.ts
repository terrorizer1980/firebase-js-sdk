...
export function enableNetwork(remoteStore: RemoteStore): Promise<void> {
 ...
 * enableNetwork().
 */
export async function disableNetwork(remoteStore: RemoteStore): Promise<void> {
  const remoteStoreImpl = debugCast(remoteStore, RemoteStoreImpl);
...
function raiseWatchSnapshot(
  ...
  debugAssert(
    !!remoteStoreImpl.syncEngine.applyRemoteEvent,
    'applyRemoteEvent() not set'
  );
  return remoteStoreImpl.syncEngine.applyRemoteEvent(remoteEvent);
}
...
async function handleTargetError(
  ...
  for (const targetId of watchChange.targetIds) {
    ...
    if (remoteStoreImpl.listenTargets.has(targetId)) {
      debugAssert(
        !!remoteStoreImpl.syncEngine.rejectListen,
        'rejectListen() not set'
      );
      await remoteStoreImpl.syncEngine.rejectListen(targetId, error);
      remoteStoreImpl.listenTargets.delete(targetId);
...
async function onMutationResult(
  ...
  debugAssert(
    !!remoteStoreImpl.syncEngine.applySuccessfulWrite,
    'applySuccessfulWrite() not set'
  );
  await executeWithRecovery(remoteStoreImpl, () =>
    remoteStoreImpl.syncEngine.applySuccessfulWrite!(success)
  );
...
async function handleWriteError(
  ...
  if (isPermanentWriteError(error.code)) {
    ...
    debugAssert(
      !!remoteStoreImpl.syncEngine.rejectFailedWrite,
      'rejectFailedWrite() not set'
    );
    await executeWithRecovery(remoteStoreImpl, () =>
      remoteStoreImpl.syncEngine.rejectFailedWrite!(batch.batchId, error)
    );
...
export async function remoteStoreHandleCredentialChange(
  ...
  remoteStoreImpl.onlineStateTracker.set(OnlineState.Unknown);
  debugAssert(
    !!remoteStoreImpl.syncEngine.handleCredentialChange,
    'handleCredentialChange() not set'
  );
  await remoteStoreImpl.syncEngine.handleCredentialChange(user);
...
function ensureWatchStream(
  ...
  if (!remoteStoreImpl.watchStream) {
    ...
    addNetworkStatusHandler(remoteStoreImpl, async enabled => {
      if (enabled) {
